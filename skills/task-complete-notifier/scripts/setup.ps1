...
function Read-SecretText {
  param([string]$Prompt)
  return ConvertFrom-SecureStringPlainText -SecureValue (Read-Host $Prompt -AsSecureString)
}

if (Test-Path -LiteralPath $envFile) {
  $overwrite = Read-Host ".env already exists. Overwrite it? Type YES to continue"
  if ($overwrite -ne "YES") { Write-Host "Setup cancelled."; exit 0 }
}
...
if ($Provider -eq "pushover") {
  ...
  @("# Local secrets. Do not commit this file.", "PUSHOVER_APP_TOKEN=$appToken", "PUSHOVER_USER_KEY=$userKey") | Set-Content -LiteralPath $envFile -Encoding UTF8
...
if ($Provider -eq "pushcut") {
  ...
  @("# Local secrets. Do not commit this file.", "PUSHCUT_WEBHOOK_URL=$webhookUrl") | Set-Content -LiteralPath $envFile -Encoding UTF8
...
if ($Provider -eq "webhook") {
  ...
  @("# Local secrets. Do not commit this file.", "WEBHOOK_URL=$webhookUrl") | Set-Content -LiteralPath $envFile -Encoding UTF8
...
if ($Provider -eq "wecom") {
  ...
  @("# Local secrets. Do not commit this file.", "WECOM_WEBHOOK_URL=$webhookUrl") | Set-Content -LiteralPath $envFile -Encoding UTF8
